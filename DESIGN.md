...
## Replay reliability, 0.9.3
Playback ranges from 4 to 30 captures per second on desktop and web. The selected web day is preloaded automatically into the bounded private cache; playback initially buffers six seconds, then decodes a small rolling window ahead of display. Its clock compensates for decoding time. Pending manifests refresh every 15 seconds while the page is visible. Uploads retain one authorization plan per transfer, use four parallel requests and checkpoint every 16 derivatives. Absolute power-of-two time buckets avoid resampling the whole day for every live capture.

The long-day browser test covers 400 full-HD synthetic frames, paired camera media, a 350 ms batch latency, no repeated downloads and no rebuffering after initial preparation. These measurements are fixture results, not a guarantee for every connection.
