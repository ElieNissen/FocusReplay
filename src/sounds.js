...
export function playSound(kind = 'tap') {
  ...
  try {
    ...
    const notes = kind === 'reminder' ? [523.25, 659.25, 783.99] : kind === 'confirm' ? [520, 720] : [640];
    context.resume().then(() => {
  ...
  } catch { /* Audio availability must never block an action. */ }
}
export function useSoundDesign(value) {
  useEffect(() => { if (value) settings = value; }, [value]);
  useEffect(() => {
    const click = (event) => {
      const control = event.target.closest?.('button, summary, [role="button"], input[type="checkbox"]');
      if (control && !control.disabled && control.getAttribute('aria-disabled') !== 'true') playSound();
    };
    const change = (event) => {
