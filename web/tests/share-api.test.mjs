...
test("profile credentials and cookies cannot cross profiles; masked and removed images revoke immediately", async () => {
  ...
  await f.config("bob");
  assert.equal(
    (await f.request("bob", "/config", "POST", { password }, f.owner("alice")))
      .status,
    401,
  );
  const a = await f.login("alice"),
    b = await f.login("bob");
  assert.equal(
    (await f.request("bob", "/snapshot", "GET", undefined, a)).status,
    401,
  );
  ...
  assert.equal(
    (await f.request("alice", "/image/" + id, "GET", undefined, a)).status,
    200,
  );
  assert.equal(
    (await f.request("bob", "/image/" + id, "GET", undefined, b)).status,
    404,
  );
  ...
  await f.config("alice");
  assert.equal(
    (await f.request("alice", "/snapshot", "GET", undefined, a)).status,
    401,
  );
  f.sql.close();
});
test("password login limits attempts, rejects cross-origin, and enforces 800 image quota", async () => {
  ...
  for (let i = 0; i < 5; i++)
    assert.equal(
      (await f.request("alice", "/login", "POST", { password: "incorrect" }))
        .status,
      401,
    );
  assert.equal(
    (await f.request("alice", "/login", "POST", { password })).status,
    429,
  );
  assert.equal(
    (
...
test('every private read rejects anonymous requests; only decorative icons are public',async()=>{
 const f=fixture();await f.config('alice');
 for(const path of ['/snapshot','/image/11111111-1111-4111-8111-111111111111'])assert.equal((await f.request('alice',path)).status,401);
 const {readdir}=await import('node:fs/promises');
 assert.deepEqual((await readdir(new URL('../public/',import.meta.url))).sort(),['favicon.svg','file.svg','globe.svg','icon.png','window.svg']);
 f.sql.close();
});
