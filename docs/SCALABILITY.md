...
## Media policy in 0.9
New captures have up to four independently authorized derivatives (public/profile × screen/camera), each limited to 12 KB: at most 38.4 MB per 800-instant profile. Legacy 50 KB images keep the conservative 40 MB profile ceiling. Six screen plus six camera public previews cost at most 144 KB per card, or 2.88 MB / 240 image reads for 20 fully visible cards. Hidden sources cost no image reads. Recent exact application intervals are capped at 5,000 and older entries are discarded; the entire manifest remains limited to 1.5 MB. Changing visibility cannot regenerate expired local originals. Shared privacy settings are controlled in both desktop and website.

## Screen quality in 0.9.2
Authorized, visible profile screens use a separate derivative version, up to 1920 px and 96,000 JPEG bytes. Public screens, blurred screens and camera images remain capped at 12,000 bytes each. With all four derivatives enabled, the ceiling is now 105.6 MB per 800-instant profile (96 KB + 3 × 12 KB), excluding metadata, temporary objects and legacy originals. This supersedes the 0.9 image-only estimates above; it does not raise the number of captures or accounts. Existing images are regenerated only while the desktop still has the originals. Uploads start with recent instants and checkpoint every 16 derivatives.

The replay keeps at most 96 MiB of compressed images and approximately 96 MiB of retained decoded pixels, plus visible canvases and bounded in-flight decoding. It preloads nearby images and 40 day landmarks; explicit full-day preload still respects these memory budgets. Temporary blur during scrubbing affects already authorized images only and is not a privacy mechanism. Server-side destructive blur remains authoritative.
